...
//            | GL.GL_TRANSFORM_BIT);
        ...
        try
        {
            ...
            if (showOutlines)
            {
                ...
                gl.glMatrixMode(GL.GL_TEXTURE);
                gl.glPushMatrix();
                gl.glTexEnvf(GL.GL_TEXTURE_ENV, GL.GL_TEXTURE_ENV_MODE, GL.GL_ADD);
            }
            ...
            for (SectorGeometry sg : dc.getSurfaceGeometry())
            {
                ...
                for (SurfaceTile tile : tilesToRender)
                {
                    ...
                    if (tile.bind(dc))
                    {
                        ...
                        gl.glScalef((float)transform.HScale, (float)transform.VScale, 1f);
                        gl.glTranslatef((float)transform.HShift, (float)transform.VShift, 0f);
...
//            gl.glPopAttrib();
    ...
    protected void initAlphaTexture(int size)
    {
        ...
        this.alphaTexture.setTexParameteri(GL.GL_TEXTURE_WRAP_S, GL.GL_CLAMP_TO_EDGE);
        this.alphaTexture.setTexParameteri(GL.GL_TEXTURE_WRAP_T, GL.GL_CLAMP_TO_EDGE);
