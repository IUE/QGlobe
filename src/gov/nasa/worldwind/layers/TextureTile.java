...
public class TextureTile extends Tile implements SurfaceTile
{
    ...
    public static synchronized MemoryCache getMemoryCache()
    {
		if (cache == null) {
			if (!WorldWind.getMemoryCacheSet().containsCache(TextureTile.class.getName())) {
				...
				cache = new BasicMemoryCache((long) (0.85 * size), size);
    ...
    public Extent getExtent(DrawContext dc)
    {
        ...
        if (extent == null){
        	extent = Sector.computeBoundingBox(dc.getGlobe(), dc.getVerticalExaggeration(), this.getSector());
        }
        return extent;
    }
    ...
    public TextureTile[] getSubTiles(Level nextLevel){
    	if (subTiles == null){
    		subTiles = createSubTiles(nextLevel);
    	}
    	return subTiles;
    }
    ...
    protected void applyResourceTextureTransform(DrawContext dc)
    {
		...
		dc.getGL().glTranslatef((float) sShift, (float) tShift, 0);
		dc.getGL().glScalef((float) oneOverTwoToTheN, (float) oneOverTwoToTheN, 1);
    }
	...
	public void setEyeDist(double eyeDist) {
		this.eyeDist = eyeDist;
	}

	public double getEyeDist() {
		return eyeDist;
	}
}
