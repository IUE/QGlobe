...
public class StarsLayer extends RenderableLayer
{
    ...
    protected FloatBuffer starsColorBuffer;
    protected FloatBuffer starsVertexBuffer;
    ...
    public void doRender(DrawContext dc)
    {
        ...
        if (this.starsVertexBuffer == null || this.rebuild)
        {
            this.loadStars();
            this.rebuild = false;
        }
        ...
        if (this.starsVertexBuffer == null)
            return;
    ...
    protected void drawWithVertexArray(DrawContext dc)
    {
        ...
        gl.glVertexPointer(3, GL.GL_FLOAT, 0, starsVertexBuffer);
        gl.glColorPointer(4, GL.GL_FLOAT, 0, starsColorBuffer);
    ...
    protected void fillVbo(DrawContext dc)
    {
        ...
        gl.glBufferData(GL.GL_ARRAY_BUFFER, this.starsColorBuffer.limit() * 4, this.starsColorBuffer, GL.GL_STATIC_DRAW);
        gl.glBindBuffer(GL.GL_ARRAY_BUFFER, glBuf[1]);
        gl.glBufferData(GL.GL_ARRAY_BUFFER, this.starsVertexBuffer.limit() * 4, this.starsVertexBuffer, GL.GL_STATIC_DRAW);

        // Add it to the gpu resource cache
        dc.getGpuResourceCache().put(this.vboCacheKey, glBuf, GpuResourceCache.VBO_BUFFERS,
        		this.starsColorBuffer.limit() * 4 + this.starsVertexBuffer.limit() * 4);
    }
    ...
    protected void loadStars()
    {
        ...
        FloatBuffer interleavedBuffer = byteBuffer.asFloatBuffer();
        ...
        if (byteBuffer.order() != ByteOrder.nativeOrder())
        {
            ...
            {
                fbuffer.put(interleavedBuffer.get(i));
            }

            fbuffer.flip();

            //Make the starsBuffer the Big-Endian buffer
            interleavedBuffer = fbuffer;
        }

        //Number of stars = limit / 6 floats per star -> (R,G,B,X,Y,Z)
        this.numStars = interleavedBuffer.limit() / 6;
        interleavedBuffer.rewind();
        this.starsColorBuffer = BufferUtil.newFloatBuffer(numStars*4);
        this.starsVertexBuffer = BufferUtil.newFloatBuffer(numStars*3);
        for (int i=0; i<numStars;i++){
        	starsColorBuffer.put(interleavedBuffer.get());
        	starsColorBuffer.put(interleavedBuffer.get());
        	starsColorBuffer.put(interleavedBuffer.get());
        	starsColorBuffer.put(1f);
        	starsVertexBuffer.put(interleavedBuffer.get());
        	starsVertexBuffer.put(interleavedBuffer.get());
        	starsVertexBuffer.put(interleavedBuffer.get());
        }
        starsColorBuffer.rewind();
        starsVertexBuffer.rewind();
    }
