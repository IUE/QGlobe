...
/**
 * Beginnings of cachable sharable buffer storage. -- They are now purging and as they are used, we round robin through buffers
 * and handle purging from either space needs or because the GPU VBO Buffer Cache purged us
 *
 * We are going to need to take a few steps yet before this is ready for public consumption- and there are several opportunities
 * to hide implementation complexity and save us from bugs.   For example , the lowest level is sort of byte level moving, the middle is managing chunks which must have the same
 * stride in bytes for elements, and the highest level is rendering with the consituent pieces.
 *
 * Also, we can use this stuff for vertex arrays too- which simplifies calling patterns from its clients.
 * In that case it can hand out chunks of data which are equivalent to the interleaved vertex array storage that the client would make itself,
 * and doesn't share things between bigger chunks.   Given a clean API to this functionality, this should save us several types of bugs-
 * but we must make sure its easy to :
 *  1) setup a VBOElementRenderer for a particular usage pattern
 *  2) diagnose rendering bugs with clients
 *  3) purge using the GPU cache
 *
 * @author ${USER}
 * @version $Id: UnifiedBuffer.java 15360 2011-04-29 17:44:28Z jburkey $
 */
...
public class UnifiedBuffer
{
    ...
    {
        return "UnifiedBuffer # of cells :"+ storageCells.size();    //To change body of overridden methods use File | Settings | File Templates.
    }
    ...
    {
        return UnifiedBuffer.GetSharedInstance(dc).getUnifiedBufferClient(elementType,sizeInVertices, inPositionCoordsPerVert,inTexCoordsPerVert,inPerVertexSizeInFloats);
    }

    protected VBOElementRenderer getUnifiedBufferClient(int elementType,int sizeInVertices,  int inPositionCoordsPerVert, int inTexCoordsPerVert,int inPerVertexSizeInFloats)
    {
        return new VBOElementRenderer(this, elementType,sizeInVertices, inPositionCoordsPerVert,inTexCoordsPerVert, inPerVertexSizeInFloats);
    }
    ...
    UnifiedBufferStorageChunk getAvailableStorage_USE_PERINSTANCE_STORAGE(int requestSizeInFloats)  // <-- can use for VertexArrrays
    {
    ...
    /**
     *  Does everything necessary to get available storage for request.
     *      - may make space and purge chunks to fulfil request
     * @param dc the current DrawContext
        ...
        UnifiedBufferStorageChunk.UnifiedBufferStorageChunkClient client, int requestSizeInFloats)
    {
        ...
        else
        {
            ...
            if (this.storageCells.size() < kNumberOfCells ) // @todo add max cells, or max space before compact and free cells
            {
     ...
     * @todo Seems like later you should purge from back, and search for empty from front?
     *   -- (so oldest in back, and you find space quicker because fresh cells are in front of list)
    *
     * @param dc the current DrawContext
