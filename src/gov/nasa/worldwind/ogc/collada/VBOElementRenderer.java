...
/**
 * Class used for rendering with UnifiedBuffer objects.
 * This class is not quite ready for public consumption, but getting close.
 * Usage:
 *             vboElementRenderer = UnifiedBuffer.GetVBOElementRenderer( dc,
                GL.GL_TRIANGLES,
    ...
    extends UnifiedBufferStorageChunk.UnifiedBufferStorageChunkClient
{
    ...
    */
    ...
    protected boolean bindVBOs(DrawContext dc)
    {
        ...
        {
            gl.glTexCoordPointer(textureCoordsPerVert, GL.GL_FLOAT, vertexSizeInBytes,0); // <- last param is buffer offset
            gl.glVertexPointer(positonCoordsPerVert, GL.GL_FLOAT, vertexSizeInBytes,2 * byteSizeFloat); // <- last param is buffer offset
        }
    ...
    {
        gl.glTexCoordPointer(textureCoordsPerVert, GL.GL_FLOAT, vertexSizeInBytes, this.chunk.coordsUploadBuffer.rewind());

        this.chunk.coordsUploadBuffer.rewind();

        // last param is buffer offset for interleaved array (TTVVV)
        gl.glVertexPointer(positonCoordsPerVert, GL.GL_FLOAT, vertexSizeInBytes,this.chunk.coordsUploadBuffer.position(2));
        gl.glDrawArrays(elementType, 0, lengthInVertices);

    }

    /**
     * Draws the elements placed in this object with VBOs (by calling glDrawRangeElements)
     * after making sure the proper buffers are bound
     *
     * @param dc the current DrawContext
     */
    ...
    public void drawWithVBO(DrawContext dc)
    {
        ...
        {
            GL gl = dc.getGL();
            gl.glDrawRangeElements(elementType, this.offsetInFloats,this.offsetInFloats+this.lengthInVertices, this.lengthInVertices,GL.GL_UNSIGNED_SHORT, 0);   //The starting point of the IBO
    ...
    public FloatBuffer beginDataPush(DrawContext dc)
    {
        if (this.chunk == null)
        {
            ...
            this.chunk              = unifiedBuffer.getAvailableStorage(dc,this,requestSizeInFloats);   // move this to later binding in push, and then be able to redo it when gpucahce is flushed
            if (this.chunk != null)
            {
    ...
    /**
     * client code that uses this renderer calls this when its done filling the buffer it obtained from beginDataPush
     * so that we can push the data to the GPU by calling our internal bufferSubData
     * caches that are now invalid.
     *
     * @param dc the current DrawContext
    ...
    public void endDataPush_SynchToGPU(DrawContext dc, boolean useVBOs)
    {
        if (useVBOs)
        {
            if (!ColladaNodeShape.DEBUG_USE_IMMEDIATE_MODE)
            {
                ...
                try
                {
                    this.bufferSubData(dc, this.offsetInFloats * byteSizeFloat, this.lengthInVertices * vertexSizeInBytes);
                ...
                finally
                {
                    ...
                    gl.glBindBuffer(GL.GL_ELEMENT_ARRAY_BUFFER,0);  // during data push we are resetting current VBO alot.
                    gl.glBindBuffer(GL.GL_ARRAY_BUFFER, 0);         // a batch like push would be cool, as we do in rendering..
                }
            }
        }
    }
    ...
    /**
     * a call back that gets called by the chunk if the chunk has been purged and we need to clear
     * caches that are now invalid.
     *
    ...
    /**
     * sets up the index buffer for the renderer.  This involves filling the index buffer with
     * offsets into the chunk buffer so that later glDrawElements or glDrawRangeElements can be called
     *
     * @param dc the current DrawContext
     */
    ...
    public void setupIndexBuffer(DrawContext dc)
    {
        ...
        if (vboIds == null)
        {
            ...
            dc.getGpuResourceCache().put(this.getVboCacheKey(), vboIds, GpuResourceCache.VBO_BUFFERS, lengthInVertices*byteSizeChar);

            if (indexBuffer == null || indexBuffer.limit() < lengthInVertices*byteSizeChar)
            {
                indexBuffer = BufferUtil.newByteBuffer(this.lengthInVertices*byteSizeChar*2);   // times two for push buffer, so we tend to re-allocate this less by overestimating what we need
            }

            indexBuffer.rewind();
            int start = this.offsetInVerts;
            for (int i = 0; i < lengthInVertices; i++)   // @TODO PERF may be able to just use one unified index buffer and use a buffer offset and length in drawElements
            {
    ...
    /**
     * checks to see if the vbo for our index buffer is ready, and then asks the chunk to see if its vbo is ready as well.
     *
     * @param dc the current DrawContext
