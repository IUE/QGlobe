...
/**
 * Responsible for rendering each node found in a Collada File.  Currently created on demand by the parser in ColladaSceneShape.
 * Uses VBOElementRenderer and UnifiedBuffers to manage VBO's.
 * @author tag
 * @version $ID$
 */
...
//             return;
    ...
    protected boolean setupTexturing(DrawContext dc, GL gl, boolean texturePushed)
    {
        ...
        if (haveDrawnWithTexture || this.textureSource == null)
        {
            if (textureEnabled)
            {
    ...
    /**
     * get the texture needed for this Collada Shape
     *          using LazilyLoadedTextures, this may start deferred image loading
       ...
       protected OGLStackHandler beginDrawing(DrawContext dc, int attrMask)
       {
           ...
           if (PERF_USE_BEGIN_DRAWING_FORPERF)
           {
               if (!DEBUG_SKIP_TEXTURING_PERFORMANCE_TESTING)
               {
                   ...
                   gl.glTexEnvi(GL.GL_TEXTURE_ENV, GL.GL_TEXTURE_ENV_MODE, GL.GL_DECAL);   /// dont blend with color, or lighting- since we dont light, its faster
...
//               gl.glEnable(GL.GL_CULL_FACE);
    ...
    {
        ShapeData shapedata = (ShapeData) getCurrentData();
        if (shapedata == null || shapedata.vboElementRenderer == null) return;
    ...
    protected void generateExtentFromCollada(DrawContext dc)
    {
        if (this.getExtent() == null)
        {
            ...
            SourceData vertexSource = sources.get("VERTEX");        // hard coded for sample data, move to loops and hashmaps
            ...
            for (int i = 0; i < numberOfElements; i++)
            {
                ...
                int vertexXIndex = i * (vertsPerTri * sourcesStride);   //   have to stride across all sources, and multiply times vertsPerTri
                int vertexYIndex = vertexXIndex + (1 * sourcesStride);  //   (because sources are interleaved, stride across a vert by striding across # of sources)
                int vertexZIndex = vertexXIndex + (2 * sourcesStride);  //    -- same as above but go to last element

                int vertexTRI_FIRSTVERTEX = positionCoordsPerVert* elementsFromPElement[vertexXIndex];   //   <p> element is kind of an overly complicated index
    ...
    protected void fillVBO(DrawContext dc)
    {
        ...
        if (shapedata.vboElementRenderer.chunk == null)
        {
            needFullDataUpdate = true;
        }
        ...
        int sourcesStride, int i, FloatBuffer pushBuffer)
    {
        ...
        if (texCoordSource != null) textureCoordOffset = texCoordSource.inputOffset;
        ...
        int texTRI_FIRSTVERTEX = texCoordsPerVert * elementsFromPElement[texXIndex];   //   <p> element is kind of an overly complicated index
        ...
        if (texcoords != null)  pushBuffer.put(texcoords[texTRI_FIRSTVERTEX]);
        else                    pushBuffer.put(0);

        if (texcoords != null)  pushBuffer.put(1.0f - texcoords[texTRI_FIRSTVERTEX + 1]);
        else                    pushBuffer.put(0);
        ...
        if (texcoords != null) pushBuffer.put(texcoords[texTRI_SECONDVERTEX]);
        else pushBuffer.put(0);
        if (texcoords != null) pushBuffer.put(1.0f - texcoords[texTRI_SECONDVERTEX + 1]);
        else pushBuffer.put(0);
        ...
        if (texcoords != null) pushBuffer.put(texcoords[texTRI_THIRDVERTEX]);
        else pushBuffer.put(0);
        if (texcoords != null) pushBuffer.put(1.0f - texcoords[texTRI_THIRDVERTEX + 1]);
        else pushBuffer.put(0);
    ...
    /**
     * Computes the transform to use during rendering to convert the unit pyramid geometry representation of
     * this Pyramid to its correct Pyramid location, orientation and scale
     *
     * @param dc the current draw context
     *
    ...
    protected final Matrix computeRenderMatrix(DrawContext dc)
    {
        ...
        Position    refPosition     = dc.getGlobe().computePositionFromPoint(referenceCenter);   // do we expire these so i can keep them or do we just always recalc?
    ...
    protected final Matrix computeExtentMatrix(DrawContext dc)
     {
         ...
         {
             matrix = matrix.multiply(Matrix.fromScale(this.getModelScale().x,this.getModelScale().y,this.getModelScale().z));
         }
