...
public class ColladaSceneShape extends AbstractGeneralShape
{
    ...
    /** service i can call from my parse thread to resolve paths.
     *  this is io / filesystem work and doesnt belong on ui thread.*/
...
//              this.pickLayer = dc.getCurrentLayer();
     ...
     */
    protected ColladaRoot readModel(URL modelUrl)       // @todo size - should we really keep hold of ColladaRoot after we've parsed?  Maybe we free it since our parsed format is so small?
    {
        try
        {
            ...
            if (fileExists && WWIO.isContentType(file, KMLConstants.COLLADA_MIME_TYPE))
            {
                ...
                finally
                {
                     ...
                     {
                         e.printStackTrace();
                         Logging.logger().log(java.util.logging.Level.SEVERE, "error closing stream in ColladaSceneShape", e);
