...
public class RectangularTessellator extends WWObjectImpl implements Tessellator
{
    ...
    protected static final HashMap<Integer, ShortBuffer> indexLists = new HashMap<Integer, ShortBuffer>();
    ...
    protected static final HashMap<Integer, Integer> minColorList = new HashMap<Integer, Integer>();
    protected static final HashMap<Integer, Integer> maxColorList = new HashMap<Integer, Integer>();
    ...
    public void endRendering(DrawContext dc)
    {
        ...
        dc.getGL().glDisableClientState(GL.GL_VERTEX_ARRAY);
        dc.getGL().glDisableClientState(GL.GL_COLOR_ARRAY);
    ...
    protected void renderVA(DrawContext dc, RectTile tile, int numTextureUnits)
    {
        ...
        gl.glDrawElements(javax.media.opengl.GL.GL_TRIANGLE_STRIP, tile.ri.indices.limit(),
            javax.media.opengl.GL.GL_UNSIGNED_SHORT, tile.ri.indices.rewind());
    }
    ...
    protected boolean renderVBO(DrawContext dc, RectTile tile, int numTextureUnits)
    {
        if (tile.ri.isVboBound || this.bindVbos(dc, tile, numTextureUnits))
        {
            ...
            dc.getGL().glDrawElements(javax.media.opengl.GL.GL_TRIANGLE_STRIP, tile.ri.indices.limit(),
                javax.media.opengl.GL.GL_UNSIGNED_SHORT, 0);
            return true;
        }
        else
        {
    ...
    protected int[] fillIndexListVbo(DrawContext dc, int density, ShortBuffer indices)
    {
        GL gl = dc.getGL();
        ...
        {
            gl.glBindBuffer(GL.GL_ELEMENT_ARRAY_BUFFER, indexListVboId[0]);
            gl.glBufferData(GL.GL_ELEMENT_ARRAY_BUFFER, indices.limit() * 2, indices.rewind(), GL.GL_STATIC_DRAW);
        }
        finally
        {
    ...
    protected void renderTileID(DrawContext dc, RectTile tile)
    {
        ...
        TextRenderer textRenderer = OGLTextRenderer.getOrCreateTextRenderer(dc.getTextRendererCache(),
            com.qq.java.awt.Font.decode("Arial-Plain-15"));
    ...
    protected void renderTrianglesWithUniqueColors(DrawContext dc, RectTile tile)
    {
        ...
        int numVertices = (density + 3) * (density + 3);
        int verticesSize = numVertices * 4;
        ...
        if (oddRowColorList.containsKey(density) && evenRowColorList.containsKey(density))
        {
            ...
            tile.minColorCode = minColorList.get(density);
            tile.maxColorCode = maxColorList.get(density);
        }
        else
        {
	        ...
	        minColorList.put(density, tile.minColorCode);
	        ...
	        for (int i = 0; i < trianglesNum; i++)
	        {
	            ...
	            if (pos > prevPos)
	            {
	                ...
	                colorsOdd.put((byte) color.getRed()).put((byte) color.getGreen()).put((byte) color.getBlue()).put((byte) 0xFF);
	            }
	            else if (pos < prevPos)
	            {
	                colorsEven.position(pos);
	                colorsEven.put((byte) color.getRed()).put((byte) color.getGreen()).put((byte) color.getBlue()).put((byte) 0xFF);
	            }
	        ...
	        maxColorList.put(density, tile.maxColorCode);
        }
        ...
        try
        {
            ...
            if (dc.getGLRuntimeCapabilities().isUseVertexBufferObject() && this.bindVbos(dc, tile, 0))
            {
                ...
                //Draw the odd rows
                gl.glColorPointer(4, GL.GL_UNSIGNED_BYTE, 0, colorsOdd.rewind());
                for (int i = 0; i < sideSize; i += 2)
                {
                    gl.glDrawElements(javax.media.opengl.GL.GL_TRIANGLE_STRIP, trianglesPerRow,
                        javax.media.opengl.GL.GL_UNSIGNED_SHORT, trianglesPerRow * i * 2);
                }

                //Draw the even rows
                gl.glColorPointer(4, GL.GL_UNSIGNED_BYTE, 0, colorsEven.rewind());
            ...
            else
            {
                ...
                {
                    gl.glDrawElements(javax.media.opengl.GL.GL_TRIANGLE_STRIP, trianglesPerRow,
                        javax.media.opengl.GL.GL_UNSIGNED_SHORT, tile.ri.indices.position(trianglesPerRow * i));
                }
    ...
    protected PickedObject resolvePick(DrawContext dc, RectTile tile, Point pickPoint)
    {
        ...
        int triangleIndex = ColorUtil.getNumColorsBetween(tile.minColorCode, colorCode) - 1;
    ...
    protected Intersection[] intersect(RectTile tile, Line line)
    {
        ...
        short[] indices = new short[tile.ri.indices.limit()];
...
//        Vec4 tR = new Vec4(a[3], a[4], a[5]);
    ...
    protected static void createIndices(int density)
    {
        ...
        int indexCount = 2 * sideSize * sideSize + 4 * sideSize - 2;
        java.nio.ShortBuffer buffer = BufferUtil.newShortBuffer(indexCount);
        short k = 0;
